      ...
      unless current_user.login_histories.last.flashd_at.try(:to_date) == Date.today
        if current_user
          ...
          # ログインした一番新しい日ー１つ前のログインした日を取得して１より大きければ
            if (login_history.logind_at.to_date - array_desc[index+1].logind_at.to_date).to_int > 1
              first_day = login_history.logind_at.to_date
              break
            end
          end
          ...
          current_user.login_histories.where(logind_at: first_day..array_asc.last.logind_at).order(logind_at: "asc").each_with_index do |login_history, index|
            # ログインが連続しているかの確認。indexで比較している為、最新のログイン日に来ればループ終了。nil対策。
            if array_asc.last.logind_at.to_date == login_history.logind_at.to_date
            break
            end
            if (array_asc[index+1].logind_at.to_date - login_history.logind_at.to_date).to_int == 1
              num += 1
            end
          end
